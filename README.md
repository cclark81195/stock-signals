...
A website that lets you pick the **Dow 30** or **S&P 500**, browse every stock with its price and recent performance, and see a **BUY / SELL / HOLD** verdict based strictly on five technical indicators. Tap any stock for its trend chart (1D, 1W, 1M, 3M, 1Y, 5Y) and a breakdown of how each indicator voted.
...
**Link straight to a stock:** the address updates as you click, e.g. `.../#sp500/NVDA`, so you can copy it and send someone a specific stock.
...
- `api/` - server functions: `constituents` (index members), `quotes` (prices + verdicts), `chart` (trend chart data)
